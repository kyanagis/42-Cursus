...
/*   Updated: 2025/09/03 06:33:50 by kyanagis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	redirect_stdout_to_outfile_or_die(t_pipex *data)
{
	int	flags;
	int	fd;
	...
	if (data->out_mode == OUT_APPEND)
		...
		flags |= O_TRUNC;
	fd = open(data->outfile_path, flags, FILE_PERMISSIONS);
	if (fd < 0)
...
static void	setup_stdout(t_pipex *data, t_exec stage)
{
	if (stage.idx < data->cmd_count - 1)
	{
		...
		redirect_stdout_to_outfile_or_die(data);
}
